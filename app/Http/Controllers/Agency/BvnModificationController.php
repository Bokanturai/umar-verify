...
class BvnModificationController extends Controller
{
    ...
    {
        try {
            $pendingSubmissions = AgentService::where('service_type', 'bvn_modification')
            ...
            $apiKey = env('AREWA_API_TOKEN');
            $apiBaseUrl = env('AREWA_BASE_URL');
            $url = rtrim($apiBaseUrl, '/') . '/bvn/modification';
            ...
            foreach ($pendingSubmissions as $submission) {
                try {
                                ...
                                try {
                                    ...
                                    if ($wallet) {
                                        ...
                                        Transaction::create([
                                            ...
                                            'performed_by'    => 'System Auto-Refund',
                                            'description'     => "Refund for failed BVN Modification",
                                            'type'            => 'credit',
                                            'status'          => 'completed',
                                            'metadata'        => [
                                    ...
                                    DB::rollBack();
                                    Log::error('BVN Modification Auto Refund Error in Batch', ['error' => $e->getMessage(), 'submission_id' => $submission->id]);
                                }
                            } else {
                                $submission->update($updateData);
                            }
                    ...
                    Log::error('Batch check error for BVN Modification ' . $submission->id . ': ' . $e->getMessage());
                    continue;
                }
            }
            ...
            Log::error('Batch Check Error (BVN Modification): ' . $e->getMessage());
            return response()->json([
                'success' => false,
